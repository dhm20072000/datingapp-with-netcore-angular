...
public class MessagesController(IMessageRepository messageRepository, IMemberRepository memberRepository) : BaseApiController
{
  ...
  [HttpGet]
  public async Task<ActionResult<PaginatedResult<MessageDto>>> GetMessagesByContaienr([FromQuery] MessageParams messageParams)
  {
  ...
  public async Task<ActionResult<IReadOnlyList<MessageDto>>> GetMessageThread([FromRoute] string recipientId)
  {
    return Ok(await messageRepository.GetMessageThread(User.GetMemberId(), recipientId));
  }

  [HttpDelete("{id}")]
  public async Task<ActionResult> DeleteMessage([FromRoute] string id)
  {
    ...
    if(message.SenderId != memeberId && message.RecipientId != memeberId) return BadRequest("You cannot delete this message");

    if(message.SenderId == memeberId) message.SenderDeleted = true;
    if(message.RecipientId == memeberId) message.RecipientDeleted = true;
